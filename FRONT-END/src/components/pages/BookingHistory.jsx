    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-6">
        {Bookings && Bookings.map((booking, index) => (
          <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden"ref={pdfRef}>
              ...
              <div className="p-6 md:w-3/4">
                <div className="flex justify-between items-start">
                  <div className='flex gap-24'>
                    <div>
                    <h2 className="text-xl font-semibold mb-2">Package : {booking.packageName}</h2>
                    <p className=" mb-2"> Scheduled ON : {new Date(booking.Date).getDate()}/
                    {new Date(booking.Date).getMonth() + 1}/
                    {new Date(booking.Date).getFullYear()}</p>
                    <p className="text-lg font-semibold ">Total price :{booking.price}</p>
                    <p className="text-lg font-semibold  pt-1">Booked Seats :{booking.seat}</p>
                    </div>
